...
const generateAccessToken = (user, roles) => {
  ...
  const { username, email, firstName, lastName, cell, cellId, base, baseId, photo } = user;
  const secret = process.env.TOKEN_SECRET || 'secret';
  return {
    token: jwt.sign({ user: { username, email, firstName, lastName, cellId, cell, baseId, base, photo, roles } }, new TextEncoder().encode(secret), {
